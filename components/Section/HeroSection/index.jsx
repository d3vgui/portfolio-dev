...
export default function HeroSection() {
  ...
  useGSAP(() => {
    ...
    tl.from(".anim-top-down", { y: -500, opacity: 0 })
      .from(".anim-left-right", { x: -500, opacity: 0, stagger: 0.1 }, "<")
  ...
  useGSAP(() => {
    ...
    const tl = gsap.timeline({
      onComplete: () => setCurrentSkill((prev) => (prev + 1) % skills.length),
    })

    tl.set(letters, { y: 20, opacity: 0 })
      .to(letters, { y: 0, opacity: 1, stagger: 0.05, duration: 0.5, ease: "back.out(1.7)" })
      .to({}, { duration: 1.5 })
      .to(letters, { y: -20, opacity: 0, stagger: 0.05, duration: 0.4, ease: "power2.in" })

  }, { dependencies: [currentSkill], scope: skillsContainerRef })

  return (
    <section ref={sectionRef} id="heroSection" className={`${styles.configSection} ${styles.heroSection}`}>
      ...
      <div className={`container d-flex flex-wrap align-items-center flex-column flex-xl-row ${styles.content}`}>
        <div className="col-lg-6 col-sm-12 col-md-12 col-xs-12 text-center text-xl-start">
          ...
          <span className={`${styles.spanWhite} anim-top-down`} style={{ display: 'inline-block' }}>
            Oi, meu nome é
          </span>
          <h2 className="anim-left-right">Guilherme Carvalho</h2>
          <h3 className="anim-left-right">Front-end Developer and Data Analyst</h3>
          
          <span ref={skillsContainerRef} className={`${styles.animatedSpan} anim-bottom-up`} style={{ minHeight: '1.5em', overflow: 'hidden', display: 'block' }}>
            {skills[currentSkill].split("").map((char, index) => (
              <span key={`${currentSkill}-${index}`} className="letter" style={{ display: "inline-block" }}>
                {char === " " ? "\u00A0" : char}
              </span>
            ))}
          </span>
